...
\definecolor{comCol}{rgb}{0.0,0.0,0.0}
\newcommand\mycommfont[1]{\footnotesize\ttfamily\textcolor{comCol}{#1}}
...
\definecolor{thetaCol}{rgb}{0.8,0.2,0.2}
\definecolor{lambdaCol}{rgb}{0.0,0.5,0.0}
\definecolor{betaCol}{rgb}{0,0.2,0.8}

\newcommand{\lp}{\textcolor{lambdaCol}{\lambda_{d_i}^{\psi(k)}}}
\newcommand{\lpuu}{\textcolor{lambdaCol}{\lambda_{d_i}^{\psi(k),(u)}}}
\newcommand{\lpum}{\textcolor{lambdaCol}{\lambda_{d_i}^{\psi(k),(u-1)}}}
...
%    \fnt{$y_{ij}^k$ = measure in subject $i$, visit $j$, region $k$.}
...
%  Initialise $\boldsymbol{\theta}^{(0)}$, $\boldsymbol{\lambda}^{(0)}$, $\boldsymbol{\beta}^{(0)}$\\
  \While{$\boldsymbol{\textcolor{thetaCol}{\theta}}$, $\boldsymbol{\textcolor{lambdaCol}{\lambda}}$, $\boldsymbol{\textcolor{betaCol}{\beta}}$ not converged}{
   \tcp*[l]{\algoFnt{Estimate \textcolor{thetaCol}{biomarker trajectories} (disease agnostic)}}
%     \For{$k=1$ to $K$}{
      ${\textcolor{thetaCol}{\theta_k\uu} = \argmin_{\textcolor{thetaCol}{\theta_k}} \textcolor{black}{\sum_{(i,j) \in \Omega_k}} \left[y_{ij}^k - g\left(f(\textcolor{betaCol}{\beta_i\um} + m_{ij}; \lpum) ; \textcolor{thetaCol}{\theta_k}\right) \right]^2 }$\\
%       ${\epsilon_k\uu = \frac{1}{|\Omega_k|} \sum_{(i,j) \in \Omega_k}    \left[y_{ij}^k - g\left(f(\beta_i\um + m_{ij}; \lpum) ; \theta_k\uu \right) \right]^2 }$\\
%     }
     \tcp*[l]{\algoFnt{Estimate \textcolor{lambdaCol}{dysfunction trajectories} (disease specific)}} 
%     \For{$d=1 \in \mathbb{D}$}{
%       \For{$l=1 \in \Lambda$}{
        ${\textcolor{lambdaCol}{\lambda_{d}^{l, (u)}} = \argmin_{\textcolor{lambdaCol}{\lambda_{d}^{l}}} \textcolor{black}{\sum_{(i,j,k) \in \Omega_{d,l}}} \left[y_{ij}^k - g\left(f(\textcolor{betaCol}{\beta_i\um} + m_{ij}; \textcolor{lambdaCol}{\lambda_{d}^{l}}) ; \textcolor{thetaCol}{\theta_k\uu} 
        \right) \right]^2 }$\\
%       }
%     }
    \tcp*[l]{\algoFnt{Estimate \textcolor{betaCol}{subject-specific time shifts}}} 
%     \For{$i=1 \in [1, \dots, S]$}{
      ${\textcolor{betaCol}{\beta_i\uu} = \argmin_{\textcolor{betaCol}{\beta_i}} \textcolor{black}{\sum_{(j,k) \in \Omega_i}} \left[y_{ij}^k - g\left(f(\textcolor{betaCol}{\beta_i} + m_{ij}; \lpuu) ; \textcolor{thetaCol}{\theta_k\uu}
      \right) \right]^2 }$\\
%     }
}
}
...
%  \includegraphics[width=\columnwidth, trim=0 20 0 35, clip]{trajDisSpaceOverlap_PCA_tadDrcTiny_JMD}
