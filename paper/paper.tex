...
We first test DKT on synthetic data, in order to assess its performance when ground truth is known. More precisely, we generate data that follows the DKT model exactly, and test the model's ability to recover the disease-specific (higher-level) and disease-agnostic (lower-level) trajectories, as well as the subject time-shifts. 
