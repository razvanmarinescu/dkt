...
\begin{itemize}
 ...
 \item Within each agnostic unit, we define the parameters $theta_0$ -- $theta_5$ corresponding to biomarker trajectories within the agnostic unit.
...
The cohort from the Dementia Research Center UK used for validation had the following demongraphics: 10 subjects diagnosed with Posterior Cortical Atrophy, with a mean age of 59.4, 40\% females, as well as 10 age-matched controls with a mean age of 59.3, 50\% females.


For the validation on TADPOLE subtypes, we used applied the SuStaIn model on TADPOLE to split the population into three subtypes with different progression: a hippocampal subtype \TODODODO
When splitting the TADPOLE subjects into three populations with different progressions, the resulting groups had the following demographics:
...
%   Cohort & Number of scans & Age & Gender (\%F)\\
%   Controls & 10 & 59.4 & 50\%\\
%   PCA & 10 & 59.3 & 40\%\\
%  \end{tabular}
% \end{table}
