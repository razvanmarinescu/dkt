...
def agregateFSDataDRC():
  ...
  subcortOutFile = '../data/DRC/drcFSXSubcort.csv'
  ...
  cortLhOutFile = '../data/DRC/drcFSXCortLh.csv'
  ...
  cortRhOutFile = '../data/DRC/drcFSXCortRh.csv'
...
def prepareData(finalDataFile, tinyData):
  ...
  tadpoleFile = '~/tadpole/neil_repo/TADPOLE_D1_D2.csv'
  ...
  selectedBiomk = [selectedBiomk[i] for i in [2,3,8,9,14,15,20,21,26,27]]
  selectedBiomk += ['CDRSB', 'ADAS13']
  print(selectedBiomk)
  # print(ads)
  ...
  # only keep the tadpole entries
  idxToDrop = np.in1d(dataDfAll.dataset, 2)
  dataDfAll.drop(dataDfAll.index[idxToDrop], inplace=True)
  dataDfAll.reset_index(drop=True, inplace=True)
...
def main():
  ...
  params['trueParams']['trueXsTrajX'] = params['trueParams']['trueDysfuncXsX']
  ...
  params['trueParams']['trueDysTrajFromDpsXU'] = [np.zeros((nrXPoints,nrFuncUnits)) for d in range(nrDis)]
